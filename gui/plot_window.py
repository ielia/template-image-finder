import cv2
import numpy
from image_filters.color_space import COLOR_BGR, COLOR_BW
...
class PlotWindow:
    ...
    def __init__(self, dimensions: tuple[int, int] = (800, 600), center: tuple[int, int] = None,
                 scale: float = INITIAL_SCALE, color_space: str = COLOR_BGR):
        ...
        cv2.resizeWindow(PlotWindow.WINDOW_NAME, dimensions[0], dimensions[1])
    ...
    def _crop_to_win_size(self):
        ...
        self._cropped_plot_image = numpy.zeros((wh, ww), numpy.uint8) if self._color_space == COLOR_BW else \
            numpy.zeros((wh, ww, 3), numpy.uint8)
    ...
    def is_open(self):
        try:
            return cv2.getWindowProperty(PlotWindow.WINDOW_NAME, 0) >= 0
        except:
            return False
    ...
    def reset_title(self):
        cv2.setWindowTitle(
            PlotWindow.WINDOW_NAME,
            f'{self._title} (scale = {self.scale * 100.0:.1f}%)' + (' Loading...' if self._is_loading else '')
        )
    ...
    def set_plot_image(self, plot_image, title: str):
        if len(plot_image.shape) > 2 and plot_image.shape[2] == 3:
            self._color_space = COLOR_BGR
        else:
            self._color_space = COLOR_BW
